...
| `/ibkr` | `.claude/skills/ouroboros-ibkr-critic/SKILL.md` | IBKR週次評価・パラメータ自動改善 | VM: ibkr_critic_report.json | 週次verdictレポート |
| `/ibkr-guardian` | `.claude/commands/ibkr-guardian.md` | IBKR日次P&L・ポジション・損失限界安全確認 | VM: ibkr_state.json, ibkr_trade_log_*.csv | 日次安全確認レポート |
| `/critic` | `.claude/skills/ouroboros-btc-critic/SKILL.md` | BTC週次評価・パラメータ自動改善 | VM: critic_report.json | 週次verdictレポート |
| `/postmortem` | `.claude/commands/postmortem.md` | シグナル事後分析・TP/SL別Edge算出・Kelly試算 | VM: `logs/trade_log_*.csv` | Edge/Kelly/DDレポート |
| `/regime` | `.claude/commands/regime.md` | 市場レジーム検出（ATRパーセンタイル・EMA・Hurst）| VM: `data/historical_ohlc.csv`, `logs/` | 4象限レジーム分類・推奨フィルター |
| `/kelly` | `.claude/commands/kelly.md` | Kelly基準Edge・推奨リスク倍率（BTC+IBKR） | VM: `logs/trade_log_*.csv`, `logs/ibkr_trade_log_*.csv` | Kelly全倍率テーブル |
...
| 日付 | 変更 | 担当セッション |
|------|------|-------------|
| 2026-05-21 | `/postmortem`, `/regime`, `/kelly` 新設（プロリポジトリ移植） | session N |
