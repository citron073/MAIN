...
def _load_module():
    module_path = Path(__file__).resolve().parents[1] / "tools" / "normalize_keiba_csv.py"
    spec = importlib.util.spec_from_file_location("normalize_keiba_csv", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("normalize_keiba_csv.py をロードできません")
