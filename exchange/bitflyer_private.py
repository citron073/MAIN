...
class BitflyerPrivateClient:
    ...
    # Retry on 429 / 5xx / network errors. 4xx (except 429) are not retried.
    _RETRY_MAX = 3
    _RETRY_BACKOFF = 1.0  # seconds; doubles each attempt (1s, 2s, 4s)
    ...
    def _request(
        ...
        *,
        ...
        for attempt in range(self._RETRY_MAX + 1):
            # Regenerate timestamp+signature on each attempt (timestamp must be fresh).
            ...
            try:
                ...
                if retryable and attempt < self._RETRY_MAX:
    ...
    def get_collateral(self) -> Dict[str, Any]:
        resp = self._request("GET", "/v1/me/getcollateral")
        if isinstance(resp, dict):
            return resp
        raise BitflyerAPIError(f"get_collateral failed: {resp}")

    def get_positions(self, *, product_code: str) -> List[Dict[str, Any]]:
        query: Dict[str, Any] = {"product_code": product_code}
        resp = self._request("GET", "/v1/me/getpositions", query=query)
        if isinstance(resp, list):
            return [x for x in resp if isinstance(x, dict)]
        raise BitflyerAPIError(f"get_positions failed: {resp}")

    def get_collateral_jpy(self) -> float:
        row = self.get_collateral()
        # bitFlyer field name is typically "collateral". Keep fallbacks for compatibility.
        for key in ("collateral", "collateral_amount", "available_collateral"):
            val = row.get(key)
            try:
                if val is not None:
                    return float(val)
                ...
                continue
        raise BitflyerAPIError(f"collateral amount not found: keys={list(row.keys())}")
