...
| 項目 | 状態 | メモ |
|------|------|------|
| **日本株スイングWF審査** | 🟢可能(研究) | データ源=**J-Quants API(無料/REST/Windows不要)**。Kuon/Ricky が機能実証。**JST日中取引で2FA/ギャップ問題を構造解決**。実弾執行はkabuステーションAPI=Windows必須(凍結RAKUTENと同じ壁)→当面PAPER or Windows環境 |
